...
func (g *GoGitClient) Clone(repo scm.Repo) error {
	printDebugGoGit("Clone", repo, fmt.Sprintf("URL: %s", repo.CloneURL))
...
func (g *GoGitClient) SetOriginWithCredentials(repo scm.Repo) error {
	printDebugGoGit("SetOriginWithCredentials", repo, fmt.Sprintf("URL: %s", repo.CloneURL))
...
func (g *GoGitClient) HasUnpushedCommits(repo scm.Repo) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get local branch reference: %w", err)
	}
...
func (g *GoGitClient) HasCommitsNotOnDefaultBranch(repo scm.Repo, currentBranch string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get current branch reference: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("failed to get default branch commit: %w", err)
	}

	currentCommit, err := r.CommitObject(currentRef.Hash())
	if err != nil {
		return false, fmt.Errorf("failed to get current branch commit: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("failed to check default ancestry: %w", err)
	}
...
func (g *GoGitClient) IsDefaultBranchBehindHead(repo scm.Repo, currentBranch string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to check if default is ancestor: %w", err)
	}
	...
	if defaultHasUniqueCommits {
		...
		if err != nil {
			return false, fmt.Errorf("failed to check if current is ancestor: %w", err)
		}
